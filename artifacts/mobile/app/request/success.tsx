    ...
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      <View style={styles.hero}>
        <Animated.View style={checkStyle}>
          <View style={[styles.successRing, { borderColor: `${colors.accentOrange}40` }]}>
            <View style={[styles.successCircle, { backgroundColor: colors.accentOrange }]}>
        ...
        <Animated.View style={[styles.textWrap, contentStyle]}>
          <Text style={[styles.title, { color: colors.primary }]}>Request Sent!</Text>
          <Text style={[styles.sub, { color: colors.secondary }]}>
            Your product request has been received. Our team will source it and update you shortly.
          </Text>
        </Animated.View>

        <Animated.View
          style={[styles.card, { backgroundColor: colors.card }, shadows.card, contentStyle]}
        >
          {[
            {
              icon: "hash" as const,
              label: "Request ID",
              value: "REQ" + Math.random().toString(36).substr(2, 5).toUpperCase(),
            },
            { icon: "clock" as const, label: "Response Time", value: "Within 30 minutes" },
            { icon: "bell" as const, label: "Notification", value: "We'll notify you when sourced" },
          ].map((item, i) => (
            <View key={item.label}>
              {i > 0 && <View style={[styles.divider, { backgroundColor: colors.border }]} />}
              <View style={styles.infoRow}>
                <View style={[styles.infoIcon, { backgroundColor: colors.muted }]}>
                  <Feather name={item.icon} size={15} color={colors.accentOrange} />
                </View>
