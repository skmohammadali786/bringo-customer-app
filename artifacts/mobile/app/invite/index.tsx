    ...
    <View style={[{ flex: 1, backgroundColor: colors.background }]}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={[styles.content, { paddingBottom: botPad }]}>
        ...
        <View style={[styles.heroCard, { backgroundColor: colors.primary }, shadows.lg]}>
          ...
          <Text style={[styles.heroTitle, { color: colors.primaryForeground }]}>Earn ₹200 for every friend</Text>
        ...
        <View style={[styles.codeCard, { backgroundColor: colors.card }, shadows.card]}>
          ...
          <View style={styles.codeRow}>
            <View style={[styles.codeBadge, { backgroundColor: colors.muted, borderColor: colors.border }]}>
              <Text style={[styles.codeText, { color: colors.primary }]}>{referralCode}</Text>
            </View>
            <Pressable onPress={handleCopy}
              style={[styles.copyBtn, { backgroundColor: copied ? colors.accentGreen : colors.primary }]}>
        ...
        <View style={styles.shareGrid}>
          {SHARE_OPTIONS.map((opt) => (
            <Pressable key={opt.id} style={[styles.shareBtn, { backgroundColor: colors.card }, shadows.sm]}>
            ...
            <View key={row.step}>
              ...
              <View style={styles.stepRow}>
                <View style={[styles.stepBadge, { backgroundColor: colors.primary }]}>
                  <Text style={[styles.stepNum, { color: colors.primaryForeground }]}>{row.step}</Text>
                </View>
        ...
        <View style={styles.sectionHeader}>
          ...
          <Pressable onPress={() => router.push("/refer/history" as any)}>
            <Text style={[styles.seeAll, { color: colors.accentOrange }]}>See all</Text>
          </Pressable>
        </View>
        {REFERRAL_HISTORY.slice(0, 2).map((r) => (
          <View key={r.name} style={[styles.refRow, { backgroundColor: colors.card }, shadows.sm]}>
            <View style={[styles.refAvatar, { backgroundColor: colors.muted }]}>
              <Text style={[styles.refInitial, { color: colors.primary }]}>{r.name.charAt(0)}</Text>
            </View>
            <View style={styles.refInfo}>
              <Text style={[styles.refName, { color: colors.primary }]}>{r.name}</Text>
              <Text style={[styles.refStatus, { color: colors.mutedForeground }]}>{r.status} · {r.time}</Text>
            </View>
            <Text style={[styles.refEarning, { color: r.earning === "Pending" ? colors.mutedForeground : colors.accentGreen }]}>
              {r.earning}
            </Text>
          </View>
        ))}
...
const styles = StyleSheet.create({
  ...
  heroTitle: { fontFamily: "Inter_700Bold", fontSize: 24, letterSpacing: -0.8, textAlign: "center" },
  heroSub: { fontFamily: "Inter_400Regular", fontSize: 14, textAlign: "center", lineHeight: 20 },
  earningsRow: { flexDirection: "row", gap: 10, marginTop: 6 },
  earningCard: { flex: 1, borderRadius: 14, padding: 12, alignItems: "center", gap: 3 },
  earningValue: { fontFamily: "Inter_700Bold", fontSize: 20, color: "#FFF", letterSpacing: -0.5 },
  earningLabel: { fontFamily: "Inter_400Regular", fontSize: 11, color: "rgba(247,245,240,0.7)" },
  codeCard: { borderRadius: 22, padding: 20, gap: 10 },
  codeLabel: { fontFamily: "Inter_400Regular", fontSize: 13 },
  codeRow: { flexDirection: "row", alignItems: "center", gap: 12 },
  codeBadge: { flex: 1, borderRadius: 12, paddingHorizontal: 14, paddingVertical: 10, borderWidth: 1.5, borderStyle: "dashed" },
  ...
  shareIcon: { width: 48, height: 48, borderRadius: 16, alignItems: "center", justifyContent: "center" },
  ...
  refRow: { borderRadius: 16, padding: 14, flexDirection: "row", alignItems: "center", gap: 10 },
